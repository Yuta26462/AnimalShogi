	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	Live2D_ModelHandle2 = Live2D_LoadModel("dll/miku/runtime/miku.model3.json");	//Live2Dモデル読み込み
	...
	Live2D_Model_SetExtendRate(Live2D_ModelHandle2, 1.6f, 1.6f);	//Live2Dモデルの大きさを設定

	ChangeFontType(DX_FONTTYPE_ANTIALIASING);		//フォントをアンチエイリアス対応にする。
	// ContentsFont = CreateFontToHandle("Cherry bomb", 30, 2, DX_FONTTYPE_ANTIALIASING, DX_CHARSET_DEFAULT);	//ContentsFontフォントデータ(メッセージ用)を作成
	...
	// MenuFont = CreateFontToHandle("Cherry bomb", 45, 2, DX_FONTTYPE_ANTIALIASING, DX_CHARSET_DEFAULT);	//ContentsFontフォントデータ(メッセージ用)を作成
	CreditFont = CreateFontToHandle("メイリオ", 15, 1, DX_FONTTYPE_ANTIALIASING, DX_CHARSET_DEFAULT);	//クレジット用フォントデータを作成
...
void DrawGameTitle(void)
{
	...
	{
		SetFontSize(50);
		DrawStringToHandle(380, 410, "す た ぁ と", 0x000000, MenuFont);
	}
	...
	if (MouseX > 370 && MouseX < 650 && MouseY>405 && MouseY < 465) {// ホバー時
		DrawStringToHandle(380, 410, "す た ぁ と", 0xffa500, MenuFont);
	}
}
...
void GameMain(void)
{
	...
	// クレジット表記
	DrawFormatStringToHandle(385, 650, 0x000000, CreditFont, "初音ミク (C) Crypton Future Media, Inc.");
	DrawFormatStringToHandle(385, 670, 0x000000, CreditFont, "桃瀬ひより (C) Live2D Inc.");
...
void GamePause(void) {
	...
	if (Pause == true) {
		...
		DrawFormatStringToHandle(430 + offsetX, 185 + offsetY, 0xfffffff, MenuFont, "ぽーず");

		// タイトルボタン
		DrawRotaGraph(505 + offsetX, 400 + offsetY, 0.8f, 0, Button, TRUE, FALSE);
		DrawFormatStringToHandle(425 + offsetX, 380 + offsetY, 0x000000, MenuFont, "たいとる");

		DrawRotaGraph(505 + offsetX, 500 + offsetY, 0.8f, 0, Button, TRUE, FALSE);
		DrawFormatStringToHandle(450 + offsetX, 480 + offsetY, 0x000000, MenuFont, "おわる");


		// マウス左クリック判定
		if (KeyFlg & MOUSE_INPUT_LEFT) {
			if (MouseX < 590 + offsetX && MouseX > 425 + offsetX && MouseY > 370 + offsetY && MouseY < 430 + offsetY) {	//タイトル画面ボタン
				PlaySoundMem(StartClick, DX_PLAYTYPE_BACK);
				StopSoundMem(TitleBGM01);
				GameState = GAME_TITLE;
			}

			if (MouseX < 590 + offsetX && MouseX > 425 + offsetX && MouseY > 470 + offsetY && MouseY < 530 + offsetY) {	//おわる画面ボタン
				PlaySoundMem(StartClick, DX_PLAYTYPE_BACK);
				GameState = END;
			}
		}
		if (MouseX < 590 + offsetX && MouseX > 425 + offsetX && MouseY > 370 + offsetY && MouseY < 430 + offsetY) {//タイトル画面ボタン(ホバー時)
			DrawFormatStringToHandle(425 + offsetX, 380 + offsetY, 0xffd700, MenuFont, "たいとる");
		}
		if (MouseX < 590 + offsetX && MouseX > 425 + offsetX && MouseY > 470 + offsetY && MouseY < 530 + offsetY) {	//おわる画面ボタン(ホバー時)
			DrawFormatStringToHandle(450 + offsetX, 480 + offsetY, 0xffd700, MenuFont, "おわる");
		}
...
void SideBar(void) {
	...
	Live2D_Model_SetTranslate(Live2D_ModelHandle2, Pflag == 1 ? -380 : 400, -50);		//Live2Dモデルの座標を設定
...
void ChangeTurn(void)
{
	if (Pause == false) {
		...
		DrawFormatStringToHandle(400 + offsetX, 185 + offsetY, 0xfffff00, MenuFont, "たーんこうたい");
		DrawFormatStringToHandle(350 + offsetX, 250 + offsetY, 0xffff00, ContentsFont, "%dP のひとへこうたいしてね！", Pflag == 1 ? 2 : 1);

		// つぎのひとへボタン
		DrawRotaGraph(535 + offsetX, 400 + offsetY, 1.4f, 0, Button, TRUE, FALSE);
		DrawFormatStringToHandle(410 + offsetX, 380 + offsetY, 0x000000, MenuFont, "%dPのひとへ", Pflag == 1 ? 2 : 1);


		// マウス左クリック判定
		if (KeyFlg & MOUSE_INPUT_LEFT) {
			if (MouseX < 680 + offsetX && MouseX > 390 + offsetX && MouseY > 345 + offsetY && MouseY < 450 + offsetY) {	//たーんこうたいボタン
				PlaySoundMem(StartClick, DX_PLAYTYPE_BACK);
		...
		if (MouseX < 680 + offsetX && MouseX > 390 + offsetX && MouseY > 345 + offsetY && MouseY < 450 + offsetY) {	//たーんこうたいボタン(ホバー時)
			DrawFormatStringToHandle(410 + offsetX, 380 + offsetY, 0xff69b4, MenuFont, "%dPのひとへ", Pflag == 1 ? 2 : 1);
		}
...
void GameClear(void) {
	...
	DrawFormatStringToHandle(280, 260, 0xffd700, MenuFont, "%dP がかちしました！", Pflag == 1 ? 1 : 2);
	...
	DrawFormatStringToHandle(440 + offsetX, 380 + offsetY, 0x000000, MenuFont, "たいとる");

	DrawRotaGraph(520 + offsetX, 500 + offsetY, 0.8f, 0, Button, TRUE, FALSE);
	DrawFormatStringToHandle(465 + offsetX, 480 + offsetY, 0x000000, MenuFont, "おわる");


	// マウス左クリック判定
	if (KeyFlg & MOUSE_INPUT_LEFT) {
		if (MouseX < 605 + offsetX && MouseX > 440 + offsetX && MouseY > 370 + offsetY && MouseY < 430 + offsetY) {	//タイトル画面ボタン
			PlaySoundMem(StartClick, DX_PLAYTYPE_BACK);
			StopSoundMem(GameClearBGM);
			GameState = GAME_TITLE;
		}

		if (MouseX < 605 + offsetX && MouseX > 440 + offsetX && MouseY > 470 + offsetY && MouseY < 530 + offsetY) {	//おわる画面ボタン
			PlaySoundMem(StartClick, DX_PLAYTYPE_BACK);
			GameState = END;
		}
	}
	if (MouseX < 605 + offsetX && MouseX > 440 + offsetX && MouseY > 370 + offsetY && MouseY < 430 + offsetY) {//タイトル画面ボタン(ホバー時)
		DrawFormatStringToHandle(440 + offsetX, 380 + offsetY, 0xffd700, MenuFont, "たいとる");
	}
	if (MouseX < 605 + offsetX && MouseX > 440 + offsetX && MouseY > 470 + offsetY && MouseY < 530 + offsetY) {	//おわる画面ボタン(ホバー時)
		DrawFormatStringToHandle(465 + offsetX, 480 + offsetY, 0xffd700, MenuFont, "おわる");
	}
}